...
def get_user_input():
    """
    Collect user input for Flow Shop Scheduling via Streamlit interface.
    """
    ...
    for i in range(int(m)):
        row = st.text_input(f"Processing times for Machine {i + 1} (separate by spaces):")
        if row:
            P.append(list(map(float, row.split())))

    # Validate dimensions for P
    ...
    # Due dates (dj)
    st.write("### Enter Due Dates (dj)")
    due_dates = st.text_input(f"Due dates for {n} jobs (separate by spaces):")
    if due_dates:
        due_dates = list(map(float, due_dates.split()))
        if len(due_dates) != n:
            st.warning(f"Please enter exactly {n} due dates.")
            return None, None, None, None, None, None
    ...
    for k in range(int(m)):
        ...
        for i in range(int(n)):
            row = st.text_input(f"Setup times for Job {i + 1} (separate by spaces, Machine {k + 1}):")
            if row:
    ...
    return P, S, due_dates, m, n, order
...
def get_user_inputz():
    ...
    # Saisie les durees des retard dj
    print("\nEntrez les durees des retard dj (séparées par des espaces) :")
    while True:
        try:
            due_dates = list(map(float, input(f"dj pour {n} jobs : ").split()))
            if len(due_dates) != n:
    ...
    return P, S, due_dates, m, n,ordre
...
############################################################################
def gantt_flowshop_with_metrics(P, S,due_dates, seq): 
    """
    ...
    for j in range(n):
        ...
        tardiness += max(0, C[-1, job_index] - due_dates[job_index])  # Tardiness for each job
...
def main():
    ...
    """
    # Get user input
    P, S, due_dates, m, n, order = get_user_input()
    ...
    TFT, TT, Cmax, C = gantt_flowshop_with_metrics(P, S, due_dates, seq)
